$(document).bind('pageinit', function(){
    ...
    var arx = /android/i,
        isAndroid = arx.test(navigator.userAgent);
    
    $.mobile.ajaxEnabled = false;
    
    
    window.nanoSecond = 1000000000;
    ...
    window.RatingModel = BaseModel.extend({
        ...
        url: function() {
            ...
            return "/rate/"+this.get('usid')+"/"+this.get("fid")+"/"+this.get("uid")+"/"+rating;
        },
    ...
    window.FileModel = BaseModel.extend({
        ...
        url: function(){
            var url = "";
            if (this.get('id_type') == 'f') {
                url = "/file-info/"+this.get("id");
            } else {
                url = "/episode-info/"+this.get("id");
            }
            this.log("URL:",url);
            return url;
        },
        initialize: function(){
            ...
            this.log("**** /RATING ****");
            this.on("change", this.updateChildren);
        },
        updateChildren: function(){
            this.log("UPDATE CHILDREN", this.attributes);
            var attributes = this.attributes.extended || this.attributes,
            ...
            this.artists.set(albums);
            this.albums.set(artists);
        },
        getArtistTitle: function() {
            var attributes = this.attributes.extended || this.attributes;
            var artist_title = attributes.artist_title;
            if (artist_title) {
                return artist_title;
            }
            var firstArtist = this.artists.at(0) || false,
                title = attributes.title;
            if(firstArtist && title && title != "undefined") {
                return firstArtist.get('artist')+" - "+title;
            }

            var basename = attributes.basename;
            if (firstArtist) {
                return firstArtist.get('artist')+" - "+ basename;
            }
            return basename;
        },
        linkSearch: function(txt) {
            var parts = txt.split("-"),
                sections = [];
            _.each(parts, function(p){
                ...
                var s = $("<span></span>"),
                    xlink = $("<a data-ajax=\"false\" href=\"/search/?q="+encodeURIComponent(p)+"\"></a>");
                    xlink.text(p);
                    s.append(xlink);
                sections.push(s.html());
            });
            return sections.join(" - ");
        },
        getArtistTitleLink: function() {
            var firstArtist = this.artists.at(0),
                title = this.get('title') || this.playing.get("title"),
                basename = this.get('basename') || this.playing.get("basename"),
                artist_title = this.get('artist_title') || this.playing.get("artist_title"),
                link = "",
                txt = "";
            if (artist_title) {
                txt = artist_title;
            }
            if(!txt && firstArtist && title && title != "undefined") {
                txt = firstArtist.get('artist')+" - "+title;
            }
            if (!txt && firstArtist) {
                txt = firstArtist.get('artist')+" - "+ basename;
            }
            return this.linkSearch(txt);
        }
    });
    ...
    window.VolumeModel = BaseModel.extend({
        ...
        volUp: function(){
            var value = parseInt(this.get("value")) || 0;
            value++;
            if (value <= 100) {
    ...
    window.PosModel = BaseModel.extend({
        ...
        inc: function() {
            var val = parseInt(this.get("value") || 0) + nanoSecond,
        ...
        seekBackword: function(){
            var total = nanoSecond * 5,
                val = parseInt(this.get("value") || 0) - total,
                left_int = parseInt(this.get("left_int") || 0) + total,
                pos_str = formatTime(val),
                left_str = formatTime(left_int);
            if (val >= 0) {
                this.set({
        ...
        seekForward: function(){
            var total = nanoSecond * 30,
                val = parseInt(this.get("value") || 0) + total,
                left_int = parseInt(this.get("left_int") || 0) - total,
                pos_str = formatTime(val),
                left_str = formatTime(left_int);
            if (val <= this.get("dur_int")) {
                this.set({
                    "value": val,
                    "pos_str": pos_str,
                    "left_int": left_int,
    ...
    window.ConductorModel = FileModel.extend({
        ...
        initialize: function(){
            ...
            this.attributes.playing.ratings = this.convertToArray(this.attributes.extended.ratings);
        ...
        onPlayingChange:function () {
            ...
            this.set(this.attributes.extended);
        ...
        syncCollection: function(key, modelType) {
            ...
            this.attributes.extended[key] = this.convertToArray(this.attributes.extended[key]);
            // Add models that are not in the collection
            _.each(this.attributes.extended[key], function(data){
                this.log("key:",key);
                var model = this[key].get(data[idAttribute]);
                if (!model) {
                    model = new modelType(data);
                    this.log("adding key:",key);
                    this[key].add(model);
                    return;
                }
                model.set(data);
    ...
    window.RatingView = BaseView.extend({
        ...
        initialize:function(){
            ...
            if (this.options.starSize && this.options.starSize == 'small') {
                this.iconSet = {
    ...
    window.VolumeButtonView = BaseView.extend({
        events: {
            "click .vol-up": "volUp",
            "click .vol-down": "volDown",
        },
        initialize: function() {
            this.model = this.options.model || new VolumeModel;
        },
        volUp: function(){
            this.model.volUp();
        },
    ...
    window.SeekButtonView = BaseView.extend({
        events: {
            "click .seek-back": "seekBackword",
            "click .seek-forward": "seekForward",
        },
        initialize: function() {
            this.model = this.options.model || new PosModel;
        },
        seekBackword: function(){
            this.model.seekBackword();
        },
        seekForward: function() {
            this.model.seekForward();
        }
    });

    window.formatTime = function(nanosecnods) {
        var seconds = Math.floor(parseInt(nanosecnods) / nanoSecond),
    ...
    window.PlayingFileView = FileView.extend({
        ...
        updateFileInfo:function(){
            this.setPageTitle();
            this.$fileInfo.html(this.fileInfoTemplate({model:this.conductor}));
            this.onImgChange();
        },
        ...
        render: function(){
            ...
            this.$ratingsTable = this.$el.find('.ratings-table-div');
            if (!this.displayAdd) {
    ...
    window.ControlView = BaseView.extend({
        ...
        initialize: function(){
            ...
            this.isNav = this.options.isNav || false;
            this.commandModel = new CommandModel();
            this.listenTo(this.conductor.pos, 'change:playingState', this.updateButton, this);
            this.listenTo(this.commandModel, 'change', this.updateButton, this);
        },
        onClickAction: function(evt) {
            ...
            this.commandModel.fetch({
                success: _.bind(this.updateConductorModel, this)
            }, {reset:true});
        },
        render: function(){
            BaseView.prototype.render.apply(this, arguments);
            var renderData = this.conductor.playing.toJSON();
            _.extend(renderData, {"isNav": this.isNav});
            this.$el.html(this.template(renderData));
            this.$playPauseImg = this.$el.find("#play-pause-img");
            this.updateButton();
            this.$el.click(".action-link", _.bind(this.onClickAction, this));
            return this;
        },
        updateConductorModel: function() {
            this.conductor.set(this.commandModel.toJSON());
            this.updateButton();
        },
        updateButton: function() {
            var playingState = this.conductor.pos.get('playingState')
    ...
    window.FooterControlView = ControlView.extend({
        debug: false,
        name: "FooterControlView",
        initialize: function(){
            ControlView.prototype.initialize.apply(this, arguments);
        },
        render: function(){
            BaseView.prototype.render.apply(this, arguments);
            this.$playPauseImg = this.$el.find("#control-pause");
            this.$uiBtnTxt = this.$playPauseImg.find(".ui-btn-text");
            this.updateButton();
            var bindOnScroll = _.bind(this.onClickAction, this);
            this.$el.find(".action-link").bind("click", bindOnScroll);
        ...
        updateButton: function() {
            ...
            this.playingState = playingState;
            if (this.playingState == 'PLAYING') {
                this.$playPauseImg.removeClass("control-play").addClass("control-pause");
                this.$uiBtnTxt.text("Pause");
            } else {
                this.$playPauseImg.removeClass("control-pause").addClass("control-play");
                this.$uiBtnTxt.text("Play");
            }
        },
        onClickAction: function(evt) {
            var $target = $(evt.target),
                cmd = $target.data('cmd') || 
                      $target.parents('a.action-link').data("cmd");
            if (!cmd) {
                return;
            }
            evt.preventDefault();
            this.commandModel.set({"cmd": cmd});
            this.commandModel.fetch({
                "success": _.bind(this.updateConductorModel, this)
            }, {reset:true});
        },
    ...
    window.ModelDebugView = BaseView.extend({
        debug: false,
        name: "ModelDebugView",
        model: null,
        initialize: function(){
        ...
        addLi:function (v, k) {
            var li = $("<li></li>"),
                txt = JSON.stringify(v, null, 4);
            li.text(k+":"+txt);
            this.lis[k] = li;
            this.$el.append(li);
            this.model.on("change:"+k, _.bind(this.updateLi, this, k), this)
        },
        updateLi: function(k, model, options, xhr) {
            var txt = JSON.stringify(this.model.get(k), null, 4);
            this.lis[k].text(k+":"+txt);
        },
        render: function(){
            this.$el.empty();
            this.lis = {};
    ...
    window.PagingCollection = BaseCollection.extend({
        debug: true,
        name: "PagingCollection",
        model: FileModel,
        offset: 0,
        limit: 10,
        load_lock: false,
        ...
        onSuccess: function(r){
            if (r.length > 0) {
                // if r.length == 0 then we are out of results so just leave it locked.
    ...
    window.HistoryCollection = PagingCollection.extend({
        debug: false,
        name: "HistoryCollection",
        url: function(){
            return "/history-data/?s="+this.offset+"&l="+this.limit;
        }
    });

    window.SearchCollection = PagingCollection.extend({
        debug: true,
        name: "SearchCollection",
        query: "",
        filter: "all",
        initialize: function(modelData, options){
            PagingCollection.prototype.initialize.apply(this, arguments);
            window.myInit.apply(this, arguments);
            if (options) {
        ...
        url: function(){
            return "/search-data/?s="+this.offset+"&l="+this.limit+"&q="+encodeURIComponent(this.query)+"&f="+this.filter;
        }
    });

    window.PagerView = BaseView.extend({
        debug: true,
        name: "PagerView",
        template: _.template($("#tpl-row").html()),
        ...
        initialize: function(){
            BaseView.prototype.initialize.apply(this, arguments);
            this.collection = this.options.collection || new PagingCollection([]);
            this.$el = $(this.options.el);
            this.el = this.options.el;
            this.collection.on("add", this.onAdd, this);
            this.$window = $(window);
            this.$document = $(document);
            this.$window.scroll(_.bind(this.loadDataWhenAtBottom, this));
        },
        load:function(div, model){
            model.fetch({
                success: _.bind(this.processLoadedData, this, div, model)
            });
        },
        processLoadedData: function(div, model){
            this.log("MODEL:", model);
            var html = this.file_info_template({model: model});
            div.find(".file-info").html(html);
            if (model.get('id_type') == 'f') {
                div.find(".add-to-preload").off("click");
                div.find(".add-to-preload").on("click", _.bind(this.onClickEncue, this, model, div));
            } else {
                div.find(".add-to-reload-div").hide();
            }
            div.find(".loading_data_for").empty();
            var ratingsTableView = new RatingsTableView({
                "$el": div.find('.ratings-table-div'),
                collection: model.ratings
            }).render();
            div.find(".album-art").hide();
            div.trigger("create");
        },
        onClickEncue:function(model, div, evt){
            evt.preventDefault();
            var cued = model.get('cued') ? false : true,
                sign = cued ? "minus" : "plus",
                $ui_icon = div.find('.ui-icon');
            if (sign == "plus") {
        ...
        onAdd: function(model){
            var div = $("<div/>");
            this.log("onAdd model.attributes:",model.attributes);
            div.append(this.template(model.attributes));
            if (this.collection.length > 10) {
                div.one("inview", _.bind(this.load, this, div, model));
            } else {
                this.load(div, model);
            }
            this.$el.append(div);
        ...
        render: function(){
            ...
            this.$window.scroll();
            this.loadDataWhenAtBottom();
            return this;
        },
        loadDataWhenAtBottom: function(){
            if (this.$window.scrollTop() + (this.$window.height() * 10) >= 
    ...
    window.RatingsPopupView = BaseView.extend({
        debug: true,
        name: "RatingsPopupView",
        ratingsTableView: null,
        $artistTitle: null,
        initialize: function(){
            BaseView.prototype.initialize.apply(this, arguments);
            this.conductor = this.options.conductor;
            this.conductor.on("all", this.updatePopupTitle, this);
        },
        updatePopupTitle: function() {
            this.$artistTitle.text("Rate - "+this.conductor.getArtistTitle());
        },
        render: function(){
            this.ratingsTableView = new RatingsTableView({
                collection:this.conductor.ratings,
                $el: this.$el.find(".footer-rating-table"),
                starSize: "small"
            });
            this.ratingsTableView.render();
            this.$artistTitle = this.$el.find(".popup-rate-artist-title");
            this.updatePopupTitle();
            return this;
        }
    });

    window.FooterView = BaseView.extend({
        debug: true,
        name: "FooterView",
        el: "#footer",
        controlView: null,
        ...
        initialize: function(){
            BaseView.prototype.initialize.apply(this, arguments);
            this.conductor = this.options.conductor;
            this.conductor.playing.on("change:basename", this.updateArtistTitle, this);
            this.conductor.ratings.on("all", this.updateRating, this);
        },
        updateArtistTitle: function() {
            this.$footerArtistTitle.html(this.conductor.getArtistTitleLink());
            this.trigger("refresh");
        },
        updateRating: function(){
            if (!this.conductor || !this.conductor.ratings || !this.conductor.ratings.length > 1) {
                this.$footerRatingEl.hide();
                return;
            }
            var model = this.conductor.ratings.at(0);
            if (!isModel(model)) {
                this.$footerRatingEl.hide();
                return;
            }
            this.$footerRatingText.text(model.get('rating'));
            this.$footerRatingEl.show();
        },
        hideFooterOnScroll:function(){
            ...
            this.showTimeout = setTimeout(_.bind(this.showFooterAfterScroll,this), 100);
        },
        showFooterAfterScroll:function(){
            this.$el.hide();
            var tmp = _.bind(function(){
                this.$el.show();
                var zIndex = this.$el.css('z-index');
                this.$el.css('z-index', parseInt(zIndex) - 100);
                this.$el.css('z-index', parseInt(zIndex) + 200);
                alert("refresh");
            }, this);
            setTimeout(tmp, 100);
        },
        render: function(){
            this.$footerRatingEl = this.$el.find(".footer-rating");
            this.$footerRatingText = this.$footerRatingEl.find(".ui-btn-text");
            this.$footerArtistTitle = this.$el.find(".artist_title");
            this.ratePopupView = new RatingsPopupView({
                "el": "#ratePopup",
                "conductor": this.conductor
            }).render();
            this.updateArtistTitle();
            this.updateRating();
            if (isAndroid) {
                // $(window).scroll(_.bind(this.hideFooterOnScroll, this));
            }
            return this;
        }
    });
