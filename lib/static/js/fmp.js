$(document).bind('pageinit', function(){
    ...
    window.FileModel = BaseModel.extend({
        ...
        linkSearch: function(txt) {
            ...
            _.each(words, function(w){
                ...
                var s = $("<span/>"),
                    xlink = $("<a data-ajax=\"false\" href=\"/search/?q="+encodeURIComponent(w)+"\"></a>");
                    xlink.text(w);
                    s.append(xlink);
    ...
    window.ControlView = BaseView.extend({
        ...
        updateConductorModel: function(model) {
            this.log("updateConductorModel:", arguments);
            delete model.attributes.cmd;
            this.conductor.set(model.attributes);
        },
    ...
    window.HistoryCollection = BaseCollection.extend({
        debug: true,
    ...
    window.HistoryView = BaseView.extend({
        debug: true,
        name: "HistoryView",
        template: _.template($("#tpl-row").html()),
        ...
        initialize: function(){
            BaseView.prototype.initialize.apply(this, arguments);
            this.collection = this.options.collection || new HistoryCollection([]);
            this.$el = $(this.options.el);
            this.el = this.options.el;
            this.collection.on("add", this.onAdd, this);
            this.$window = $(window);
        ...
        onAdd: function(model){
            var div = $("<div/>");
            this.log("model.attributes:",model.attributes);
    ...
    $("[data-role=footer]").fixedtoolbar({ disablePageZoom: false });
    $("[data-role=header]").fixedtoolbar({ tapToggleBlacklist: ".star-rating, img, a, button, input, select, textarea, .ui-header-fixed, .ui-footer-fixed" });
    $("[data-role=footer]").fixedtoolbar({ tapToggleBlacklist: ".star-rating, img, a, button, input, select, textarea, .ui-header-fixed, .ui-footer-fixed" });
});
