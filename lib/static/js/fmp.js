$(function(){
  ...
  window.RatingsCollection = Backbone.Collection.extend({
      model: window.RatingModel
  });

  window.ArtistsCollection = Backbone.Collection.extend({
      model: window.ArtistModel
  });

  window.GenresCollection = Backbone.Collection.extend({
      model: window.GenreModel
  });
  ...
  window.FileModel = Backbone.Model.extend({
      ...
      defaults: function () {
        return {
            albums: function() { return new AlbumsCollection; },
            artists: function() { return new ArtistsCollection; },
            basename: "",
            dirname: "",
            fid: "",
            genres: function() { return new GenresCollection; },
            ratings: function() { return new RatingsCollection; },
            title: "",
            cued: false
        };
      },
      getArtistTitle: function(){
          ...
          if (this.artists) {
              artist = _.first(this.artists.at(0)).get('artist') || "";
          }
          if (artist && title) {
            return artist+" - "+title;
          }
          if (artist) {
            return artist+" - "+basename;
          }
          return basename || "Error fetching title";
      },
      constructor: function() {
        this.attributes = this.defaults();
        Backbone.Model.apply(this, arguments);
        console.log("constructor arguments:@@@@@@@@@@@", arguments);
        if (arguments && !_.isUndefined(arguments[0])) {
          this.initModel('ratings', arguments[0].ratings, window.RatingModel,
                         this.attributes['ratings']);
          this.initModel('artists', arguments[0].artists, window.ArtistModel,
                         this.attributes['artists']);
          this.initModel('genres', arguments[0].genres, window.GenreModel,
                         this.attributes['genres']);
        }
      },
      initModel: function(key, objs, model, collection) {
        if (objs) {
            console.log("initModel MODEL:",model);
            _.each(objs, function(data){
                console.log("")
                var mobj = new model(data);
                collection.add(mobj);
            }, this);
        } else {
            console.log(" NO OBJS:",key)
        }
      }
  });
  ...
  window.MediaPlayerModel = window.FileModel.extend({
      ...
      initialize: function() {
          var obj = this.toJSON()
          console.log("init playerModel");
          this.playerModel = new FileModel(obj.player);
          console.log("init playingModel");
          this.playingModel = new FileModel(obj.playing);
      ...
      constructor: function() {
        Backbone.Model.apply(this, arguments);
        this.attribuest = this.defaults();
        console.log("constructor arguments:", arguments);
        _.each(arguments, function (v,k) {
            console.log("arguments["+k+"]:",v);
        }, this);
        if (arguments && !_.isUndefined(arguments[0].playing)) {
          this.initModel('ratings', arguments[0].playing.ratings, window.RatingModel);
          this.initModel('artists', arguments[0].playing.artists, window.ArtistModel);
          this.initModel('genres', arguments[0].playing.genres, window.GenreModel);
        }
      },
  ...
  window.RatingRowView = Backbone.View.extend({
      ...
      initialize: function(){
          ...
          if (this.options.ratingModel) {
              this.ratingModel = this.options.ratingModel;
  ...
  window.RatingsTableView = Backbone.View.extend({
      ...
      initialize: function() {
        ...
        console.log("RatingsTableView.ratings:",this.ratings);
      },
      render: function() {
        ...
        _.each(this.ratings, function(ratingModel){
  ...
  window.FileView = Backbone.View.extend({
    ...
    updateRatings: function() {
        this.ratingsTableView.update(this.ratings);
    },
    ...
    render: function() {
      ...
      console.log("FileView model:",this.model);
      console.log("FileView model this.model.get('ratings')",this.model.get('ratings'));
      
      this.$el.html(this.template(args));
      this.$fileInfoEl = this.$el.find('.file-info');
      this.updateFileInfo(args, true);
      this.ratingsTableView = new RatingsTableView({
         "ratings": this.attributes.ratings,
